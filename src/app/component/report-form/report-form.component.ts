    ...
    this.ngxCsvParser.parse(files[0], { header: this.header, delimiter: ',' })
      .pipe().subscribe((result: Array<any>) => {
        ...
        for (let data of this.csvRecords) {
          ...
          transformedContent = transformedContent.replace(/r_url/gi, `<a href='${data[1]}'>${data[1]}</a>`);
          transformedContent = transformedContent.replace(/sample_url/gi, `<a href='${data[2]}'>${data[2]}</a>`);
